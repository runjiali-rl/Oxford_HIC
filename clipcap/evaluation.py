...
def predict(image, model，prompt=None, args):
    ...
    bos_token_id = model.gpt2.config.bos_token_id
    num_beams = model.gpt2.config.num_beams
    length_penalty = model.gpt2.config.length_penalty
    early_stopping = model.gpt2.config.early_stopping
    num_beam_groups =  model.gpt2.config.num_beam_groups
    do_sample = model.gpt2.config.do_sample
    num_return_sequences = (
         model.gpt2.config.num_return_sequences
    )
    logits_processor = LogitsProcessorList()
    eos_token_id = model.gpt2.config.eos_token_id

    if eos_token_id is None and hasattr(model.gpt2.config, "decoder"):
...
def main():
    ...
    predicted_caption_df = pd.DataFrame(predicted_caption)
    predicted_caption_df.to_csv(save_path)
